...
# This function generates line plot depicting different kind of electric power consumption changed over time
plot_3 <- function(){
    ...
    plot(Time,Sub_metering_1, type = "l", xlab = xlab, ylab = ylab)
    lines(Time,Sub_metering_2, col = "red")
    lines(Time,Sub_metering_3, col = "blue")
    legend_cols = c("black","blue","red")
    legend_nams = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3") 
    legend("topright", lty = 1, lwd = 1, col = legend_cols, legend = legend_nams)
    par(opar)
